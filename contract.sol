...
contract GettingStartedFunctionsConsumer is FunctionsClient, ConfirmedOwner {
     ...
     */
    constructor(string memory _baseUrl) FunctionsClient(router) ConfirmedOwner(msg.sender) {
        baseUrl = _baseUrl;
    }
    ...
    function registerSelf() external payable {
        ...
        // now this contract can also use our SDK
    }

    function unregisterSelf() external onlyVerifiedUser payable {
        uint l = verifiedUsers.length;
        uint idx;
        for (idx = 0; idx < l; idx++) 
        {
    ...
    // TODO: write methods to call the api and do stuff
